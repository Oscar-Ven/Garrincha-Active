...
function CategoryBadge({ category }: { category: RewardCategory }) {
  const colors: Record<RewardCategory, string> = {
    ...
    FREE_SESSION: 'bg-green-900/40 text-green-300 border-green-700/50',
    FOOD_DRINK: 'bg-orange-900/40 text-orange-300 border-orange-700/50',
    TOURNAMENT_ENTRY: 'bg-yellow-900/40 text-yellow-300 border-yellow-700/50',
      ...
      <button
        ...
        className={cn(
          ...
          isActive
            ...
            : 'bg-slate-700/60 text-slate-400 border border-slate-600/50 hover:bg-slate-700'
        )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">Rewards</h1>
          <p className="mt-1 text-sm text-slate-400">
            Manage the rewards catalog — create, toggle, and edit rewards.
          </p>
        </div>
      ...
      {/* ── Table ── */}
      {rewards.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-slate-800 bg-slate-900/40 py-20 text-center">
          ...
          <p className="text-xs text-slate-500">Create a reward to get started.</p>
        </div>
      ) : (
                ...
                {rewards.map((reward) => (
                  ...
                  >
                    ...
                    <td className="max-w-[200px] px-4 py-3">
                      <p className="truncate font-medium text-white">{reward.title}</p>
                    ...
                    <td className="whitespace-nowrap px-4 py-3">
                      <span
                        className={cn(
                          ...
                          reward.stock === 0 ? 'text-red-400' : 'text-slate-200'
                        )}
                      >
                    ...
                    <td className="whitespace-nowrap px-4 py-3">
                      <ToggleActiveForm
                        ...
                        toggleAction={toggleRewardActive}
                      />
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-slate-400">
                      {reward.expiresAt ? (
                        <span
                          className={cn(
                            reward.expiresAt < new Date() ? 'text-red-400' : 'text-slate-400'
                          )}
                        >
                    ...
                    <td className="whitespace-nowrap px-4 py-3">
                      <Link
                        ...
                        className="inline-flex items-center gap-1 rounded-md border border-slate-700 bg-slate-800 px-2.5 py-1 text-xs font-medium text-slate-300 transition-colors hover:border-green-600 hover:bg-green-900/20 hover:text-green-300"
                      >
                        <svg
                          className="h-3 w-3"
                          fill="none"
